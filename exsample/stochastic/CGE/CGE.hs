...
-- | 一期の算出を取得する
getTermProductionTotal :: CountUnit -> World s -> Term -> Entity -> ST s Double
getTermProductionTotal u wld t e = do
    pt <- readURef (_prices wld)
    le <- readURef (_ledger wld)
...
-- | 一期の生産量の総量を取得する
getTermProduction :: CountUnit -> World s -> Term -> Entity -> Product ->ST s Double
getTermProduction u wld t e c = do
    pt <- readURef (_prices wld)
    le <- readURef (_ledger wld)
    let temp = EJ.projWithNoteBase [(Production,t)] [Not:<(Products,c,e,(.#))]
             $ termJournal t le
    case u of
        Yen     -> return $ norm $ EJT.transfer temp $ toCashTable pt
        Amount  -> return $ norm $ EJT.transfer temp $ toAmountTable pt
